...
int main(int argc, const char** argv)
{
    ...
    try {
        ...
        DenmApplication denm_app(positioning, trigger.runtime());
        denm_app.set_interval(std::chrono::milliseconds(vm["denm-interval"].as<unsigned>()));
        denm_app.print_received_message(vm.count("print-rx-denm") > 0);
        denm_app.print_generated_message(vm.count("print-tx-denm") > 0);
        context.enable(&denm_app);
